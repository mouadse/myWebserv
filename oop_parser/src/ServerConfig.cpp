...
ServerConfig::ServerConfig()
    : _port(0), _host(0), _server_name(""), _root(""),
      _client_max_body_size(kDefaultMaxBodySize), _index(""), _autoindex(false),
      _error_pages(), _locations(), _server_address(), _listen_fd(-1) {
  initErrorPages();
}
...
ServerConfig::ServerConfig(const ServerConfig &other)
    : _port(other._port), _host(other._host), _server_name(other._server_name),
      _root(other._root), _client_max_body_size(other._client_max_body_size),
      _index(other._index), _autoindex(other._autoindex),
...
static std::string normalizeWithSemicolon(std::string value,
  ...
  for (size_t i = 0; i + 1 < parameter.size(); i += 2) {
    ...
    if (ConfigFile::getTypePath(path) != 2) {
      if (ConfigFile::getTypePath(_root + path) != 1)
        throw ConfigError("Incorrect path for error page file: " + _root +
                          path);
      if (ConfigFile::checkFile(_root + path, 0) == -1 ||
          ConfigFile::checkFile(_root + path, 4) == -1)
        throw ConfigError("Error page file :" + _root + path +
                          " is not accessible");
    }
...
void ServerConfig::setLocation(std::string path,
  ...
  for (size_t i = 0; i < parameter.size(); i++) {
    ...
    } else if ((parameter[i] == "allow_methods" || parameter[i] == "methods") &&
               (i + 1) < parameter.size()) {
      if (flag_methods)
  ...
  if (new_location.getPath() != "/cgi-bin" &&
      new_location.getIndexLocation().empty())
...
bool ServerConfig::isValidErrorPages() {
  ...
  for (it = _error_pages.begin(); it != _error_pages.end(); it++) {
    if (it->first < 100 || it->first > 599)
      return (false);
    if (ConfigFile::checkFile(getRoot() + it->second, 0) < 0 ||
        ConfigFile::checkFile(getRoot() + it->second, 4) < 0)
      return (false);
  }
  return (true);
}
  ...
  if (location.getPath() == "/cgi-bin") {
    ...
    if (ConfigFile::checkFile(location.getIndexLocation(), 4) < 0) {
      ...
      if (ConfigFile::getTypePath(path) != 1) {
        ...
        path = std::string(root) + location.getPath() + "/" +
               location.getIndexLocation();
      }
      if (path.empty() || ConfigFile::getTypePath(path) != 1 ||
          ConfigFile::checkFile(path, 4) < 0)
        return (1);
    }
    if (location.getCgiPath().size() != location.getCgiExtension().size())
      return (1);
    std::vector<std::string>::const_iterator it;
    for (it = location.getCgiPath().begin(); it != location.getCgiPath().end();
         ++it) {
    ...
    if (ConfigFile::isFileExistAndReadable(location.getRootLocation() +
                                               location.getPath() + "/",
                                           location.getIndexLocation()))
      return (5);
...
ServerConfig::getLocationKey(const std::string &key) const {

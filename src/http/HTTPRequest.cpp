...
void HTTPRequest::addData(const std::vector<char>& new_chunk)
{
    if (new_chunk.empty())
        return;
    buffer.insert(buffer.end(), new_chunk.begin(), new_chunk.end());
...
bool HTTPRequest::tryParseHeaders()
{
    ...
    if (!found_end)
        return (false);
    std::string headers_block(buffer.begin() + parse_offset, buffer.begin() + pos);
...
bool HTTPRequest::parseChunkSizeLine()
{
    ...
    std::vector<char>::iterator it = std::search(
        buffer.begin() + parse_offset, buffer.end(),
        crlf.begin(), crlf.end()
    );
    if (it == buffer.end())
        return false;
